...
- *single_run*: This function creates a [Diagram_Random](Diagram_Random) object following the instructions of the configuration file and uses it to evaluate the analytical values of the magnetizations and those obtained with the MC estimators. Once it finishes it prints on terminal the time taken to perform the simulation and both the MC and the analytical values of the two magnetizations. It also prints the modulus of the difference between the MC estimated value and the analytical one.
